...
const CustomerForm = ({ isEdit = false, selected }: Prop) => {
...
//   console.log("====================================");
const selectedCustomerData = selected
? customerList.find((customer) => customer.id === parseInt(selected.id))
: undefined;
  console.log('====================================');
  console.log(selectedCustomerData);
  console.log('====================================');
  const customerCode = selectedCustomerData?.code;
  const onFinish = async (values: any) => {
    
    if (isEdit && selected) {
      dispatch(updateCustomer({ id: selected.id, gender: values.gender, phoneNumber:values.phoneNumber, dateOfBirth: values.dateOfBirth, email: values.email,firstName:values.firstName,lastName:values.lastName,code:customerCode}))
          .unwrap()
          .then((response) => {
  ...
  const initialFormValues = isEdit ? normalizeDateValues(selectedCustomerData) || {} : {};
  ...
  return (
    ...
    >
      <Card title="Thông tin khách hàng" className="form__product__card">
        <Row gutter={[20, 0]}>
          <Col xl={12}>
            <Form.Item
              name="firstName"
